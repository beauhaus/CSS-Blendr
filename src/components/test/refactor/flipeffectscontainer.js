...
const FlipEffectsContainer = (props) => {
  const { addImageMode, currentTop, flipTriggerVal } = useContext(AppContext);
  ...
  useEffect(() => {
    ...
    tl.to(flipArrows, { duration: 1, opacity: 1, fillOpacity: 0.3 }, "-=0").to(
      arrowGlow,
      { duration: 0.8, opacity: 0, fillOpacity: 0 },
      "-=.8"
    );
  }, [flipTriggerVal]);
  ...
  useEffect(() => {
    ...
    if (addImageMode) {
      tl.to(secondaryTrimBot, 1, { stopOpacity: 1 }, "-=0")
      ...
      tl.to(secondaryTrimBot, 0.5, { stopOpacity: 0 }, "-=0").to(
        secondaryTrimTop,
        0.5,
        { stopOpacity: 0 },
        "-=.5"
