...
const UsrImgThumb = ({ imgObj }) => {
  const { setUsrImgArray, setCurrentTop, currentTop } = useContext(AppContext);
  ...
  const imgClickHandler = (imgObj) => setCurrentTop(imgObj);
  // TODO: add desc: key/val to objects
  // className={(currentTop.id === imgObj.id)? 'img-selected': 'default-img'}
  return (
    <StyledUsrImgThumbContainer
      className={currentTop.id === imgObj.id ? "img-selected" : "uploaded-img"}
    >
      <button
        className="usr-img-btn img-btn"
        value={imgObj}
      ...
      <button
        className="usr-img-delete"
        onClick={() => deleteUsrImage(imgObj.id)}
      >
        <p>X</p>
      </button>
    </StyledUsrImgThumbContainer>
